...
func TestEngine_AssessSignals(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			sig:          risk.Signals{Video: 0.4, VideoOK: true, Audio: 0.4, AudioOK: true, Temporal: 0.4, TemporalOK: true},
			wantScore:    0.4,
			wantVerdict:  risk.VerdictSuspicious,
			wantVideo:    float64Ptr(0.4),
			wantAudio:    float64Ptr(0.4),
			wantTemporal: float64Ptr(0.4),
			wantReasons:  nil,
		},
		{
			name:         "all three at the likely-fake threshold is likely fake",
			sig:          risk.Signals{Video: 0.7, VideoOK: true, Audio: 0.7, AudioOK: true, Temporal: 0.7, TemporalOK: true},
			wantScore:    0.7,
			wantVerdict:  risk.VerdictLikelyFake,
			wantVideo:    float64Ptr(0.7),
			wantAudio:    float64Ptr(0.7),
			wantTemporal: float64Ptr(0.7),
			wantReasons: []string{
				"Audio signal indicates likely synthetic speech",
				"Video signal indicates likely synthetic or manipulated content",
		...
		},
		{
			...
			sig:          risk.Signals{Temporal: 0.7, TemporalOK: true},
			wantScore:    0.7,
			wantVerdict:  risk.VerdictLikelyFake,
			wantVideo:    nil,
			wantAudio:    nil,
